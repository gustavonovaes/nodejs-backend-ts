...
describe("server", () => {
  ...
  it("campos do objeto de request", async () => {
    const server = createServerMock({
      "GET /test": async (req: IServerRequest, res: IServerResponse) => {
        res.send({
          method: req.method,
          url: req.url,
          headers: req.headers,
  ...
  it("retorna internal server error para exceptions não tratadas", async () => {
    const server = createServerMock({
      "GET /error": async (_req: IServerRequest, _res: IServerResponse) => {
  ...
  it("retorna internal server error para promises rejeitadas", async () => {
    const server = createServerMock({
      "GET /rejected": async (_req: IServerRequest, _res: IServerResponse) => {
  ...
  it("encerra o servidor corretamente ao receber sinal SIGINT", async () => {
    const server = createServerMock({
      "GET /": async (_req: IServerRequest, res: IServerResponse) => {
        process.emit("SIGINT");
        res.send({ status: "ok" });
      },
    });

    listenWithGracefulShutdown(mockServer, {
      port: 0,
      host: "localhost",
    });
  ...
  it("encerra o servidor corretamente ao receber sinal SIGTERM", async () => {
    ...
    listenWithGracefulShutdown(mockServer, { port: 0, host: "localhost" });
